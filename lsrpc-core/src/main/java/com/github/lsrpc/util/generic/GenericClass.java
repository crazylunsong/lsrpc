package com.github.lsrpc.util.generic;
    ...
    public GenericClass(Class clazz, Map<String, ? extends Type> parent) {
        ...
        if (variables.length > 0) {
            for (TypeVariable<Class> variable : variables) {
                if (parent == null) {
                    childType.addVariable(new GenericType.Variable(variable.getName(), variable.getBounds()[0]));
                } else {
                    childType.addVariable(new GenericType.Variable(variable.getName(), parent.get(variable.getName())));
                }
            }
        ...
        if (parentType instanceof ParameterizedType) {
            ...
            for (int i = 0; i < arguments.length; i++) {
                ...
                if (argument instanceof Class) {
                    //Class
                    result.addVariable(new GenericType.Variable(name, argument));
                } else if (argument instanceof TypeVariable) {
                    //从子类获取泛型定义
                    result.addVariable(new GenericType.Variable(name, childType.getVariable(((TypeVariable) argument).getName())));
                } else if (argument instanceof ParameterizedType) {
                    result.addVariable(new GenericType.Variable(name, compute(argument, null, childType).getGenericType()));
        ...
        if (type instanceof Class) {
            ...
            if (gd instanceof Class) {
                ...
                if (declaringType != null) {
                    GenericType.Variable variable = declaringType.getVariable(name);
                    if (variable != null) {
                ...
                genericType.addVariable(new GenericType.Variable(name, typeVariable));
